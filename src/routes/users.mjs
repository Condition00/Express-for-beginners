...
router.get('/api/users', query('filter').
    ...
    .withMessage('Filter must be a string with length between 3 and 10 characters'),
     ...
     (request, response) => {
        ...
        // request.session.id is the session id, it is generated by express-session middleware

        //sessionStore is the store where the session data is stored
        // we can use it to retrieve the session data

        // so that is how the session stores the session object data
        // its stored in a data structure in the memory
